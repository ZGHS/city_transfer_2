...
def get_mercator(types, a, b, c, d):
    num = 0
    # # Xuezhang
    #     ak=prPXBWF3Kc3qAv68MZizNKjqUYZucdGt
    # # Zhangganghua
    #     ak=T0dSjjNVzELAaG4VDIQU7xhQDqrGFYqL
    # # Natao
    #     ak=aAqjR5cD7lINurRz4DUWnfbwPvFdl5Xj
    # # Zhoushimeng
    #     ak=MkHEZ6eafrkUXg4VGaeQpXs2k5TriWQu
    api_aks = ["prPXBWF3Kc3qAv68MZizNKjqUYZucdGt",
               "T0dSjjNVzELAaG4VDIQU7xhQDqrGFYqL",
               "aAqjR5cD7lINurRz4DUWnfbwPvFdl5Xj",
               "MkHEZ6eafrkUXg4VGaeQpXs2k5TriWQu"]
    pre_api = "http://api.map.baidu.com/place/v2/search?query=" + types + "&bounds=" + str(b) + "," + str(
        a) + "," + str(d) + "," + str(
        c) + "&page_num=" + str(num) + "&output=json&page_size=1&ak="

    req = requests.get(pre_api + api_aks[API_AK_INDEX])
    content = req.json()
    if content['status'] == 0:
...
def run():
    ...
    m_poi_type = ['food', 'hotel', 'shopping', 'life_service', 'beauty', 'view', 'entertainment', 'sport', 'edu',
                  ...
                  'entrance', 'nature']
    for i in range(0, len(m_poi_type)):
        print(i)
